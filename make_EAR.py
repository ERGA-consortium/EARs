...
logging.basicConfig(filename='EAR.log', level=logging.ERROR)

EAR_version = "v28.06.20_alpha"


# Read the content of the EAR.yaml file
with open("EAR.yaml", "r") as file:
    yaml_data = yaml.safe_load(file)



#logger.info("All required fields are present. Proceeding with the report generation...")
...
if missing_fields:
    print(f"# GENERAL INFORMATION section in the yaml file is missing or empty for the following information: {', '.join(missing_fields)}")
    sys.exit(1)

# Check that "Species" field is a string
if not isinstance(yaml_data["Species"], str):
    print(f"# GENERAL INFORMATION section in the yaml file contains incorrect data type for 'Species'. Expected 'str' but got '{type(yaml_data['Species']).__name__}'.")
    sys.exit(1)
...
if profiling_data:
    ...
    for tool in tools:
        ...
        if not tool_data:
            print(f"# PROFILING section in the yaml file is missing or empty for {tool} information.")
            sys.exit(1)

        # Check for required fields and print an error message if any are missing
        for field in required_fields:
            if not tool_data.get(field):
                print(f"# {tool} information in the PROFILING section in the yaml file is missing or empty for {field} information.")
                sys.exit(1)
        ...
        if tool == 'GenomeScope':
            genomescope_version = tool_version
            genomescope_folder = tool_data['results_folder']
        else:
            smudgeplot_version = tool_version
            smudgeplot_folder = tool_data['results_folder']

else:
    print('Warning: No profiling data found in the YAML file.')
    sys.exit(1)


# get values from PROFILING
if profiling_data:
    genomescope_data = profiling_data.get('GenomeScope')
    if genomescope_data:
        genomescope_version = genomescope_data.get('version', 'NA')
        genomescope_folder = genomescope_data.get('results_folder')
    else:
        print('Warning: No data found for GenomeScope in the YAML file.')

    smudgeplot_data = profiling_data.get('Smudgeplot')
    if smudgeplot_data:
        smudgeplot_version = smudgeplot_data.get('version', 'NA')
        smudgeplot_folder = smudgeplot_data.get('results_folder')
    else:
        print('Warning: No data found for Smudgeplot in the YAML file.')
else:
    print('Warning: No profiling data found in the YAML file.')
...
def get_completeness_value(dir_path, order, tool, haplotype):
    try:
        ...
        print(f"Error reading {dir_path}: {str(e)}")
        return ''
...
def get_png_files(dir_path):
    ...
    if len(png_files) < 4:
        print(f"Warning: Less than 4 png files found in {dir_path}. If this is diploid, some images may be missing.")
    return png_files[:4]    
